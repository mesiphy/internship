...
## 定义
Max tokens 是 大模型 / LLM 中需要被采集、处理、表示或消费的数据对象，服务于 生成式模型的输入、输出、能力控制与成本权衡。

## 核心问题
核心问题是：Max tokens 如何被获取、处理、表示、利用，以及它会怎样影响下游效果。

## 关键关系
- 所属结构：[[知识卡/原理层/大模型_LLM/0_大模型_LLM|大模型 / LLM]]
- 同目录知识：[[知识卡/原理层/大模型_LLM/Top-p|Top-p]]、[[知识卡/原理层/大模型_LLM/幻觉|幻觉]]
