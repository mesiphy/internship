...
## 定义
Tool Result Compression 是 Tool Calling / Function Calling / MCP / Connectors 中常见的工程处理策略，用来稳定 工具接入、调用闭环、权限与执行控制 对应的链路表现。

## 核心问题
核心问题是：什么时候需要用 Tool Result Compression、会带来什么代价、该如何与上下游策略配合。
...
- 同目录知识：[[知识卡/应用工程层/Tool Calling_Function Calling_MCP_Connectors/Tool Result Filtering|Tool Result Filtering]]、[[知识卡/应用工程层/Tool Calling_Function Calling_MCP_Connectors/幂等|幂等]]
