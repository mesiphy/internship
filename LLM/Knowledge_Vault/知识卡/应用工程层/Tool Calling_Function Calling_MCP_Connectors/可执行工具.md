...
## 定义
可执行工具 是 Tool Calling / Function Calling / MCP / Connectors 中的关键知识点，用于理解和组织 工具接入、调用闭环、权限与执行控制。

## 核心问题
核心问题是：可执行工具 在 Tool Calling / Function Calling / MCP / Connectors 里承担什么作用、与哪些上下游环节相连、如何判断它是否发挥作用。
...
- 同目录知识：[[知识卡/应用工程层/Tool Calling_Function Calling_MCP_Connectors/可写工具|可写工具]]、[[知识卡/应用工程层/Tool Calling_Function Calling_MCP_Connectors/数据库连接|数据库连接]]
