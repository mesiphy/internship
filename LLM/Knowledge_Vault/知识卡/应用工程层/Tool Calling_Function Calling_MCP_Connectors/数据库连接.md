...
## 定义
数据库连接 是 Tool Calling / Function Calling / MCP / Connectors 中需要被采集、处理、表示或消费的数据对象，服务于 工具接入、调用闭环、权限与执行控制。

## 核心问题
核心问题是：数据库连接 如何被获取、处理、表示、利用，以及它会怎样影响下游效果。
...
- 同目录知识：[[知识卡/应用工程层/Tool Calling_Function Calling_MCP_Connectors/可执行工具|可执行工具]]、[[知识卡/应用工程层/Tool Calling_Function Calling_MCP_Connectors/CRM连接|CRM连接]]
