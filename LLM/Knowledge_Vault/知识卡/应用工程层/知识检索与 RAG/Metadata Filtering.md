...
## 定义
Metadata Filtering 是 知识检索与 RAG 中需要被采集、处理、表示或消费的数据对象，服务于 知识召回、排序、证据注入与知识更新。

## 核心问题
核心问题是：Metadata Filtering 如何被获取、处理、表示、利用，以及它会怎样影响下游效果。

## 关键关系
- 所属结构：[[知识卡/应用工程层/知识检索与 RAG/0_知识检索与 RAG|知识检索与 RAG]]
- 同目录知识：[[知识卡/应用工程层/知识检索与 RAG/Metadata|Metadata]]、[[知识卡/应用工程层/知识检索与 RAG/Reranking|Reranking]]
